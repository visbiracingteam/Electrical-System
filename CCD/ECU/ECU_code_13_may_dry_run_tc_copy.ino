...
/*Range of sensors*/
const uint16_t apps1_gmax = 1023;
const uint16_t apps1_gmin = 0;
const uint16_t apps2_gmax = 1023;
...
const uint16_t apps1_max = 560;
const uint16_t apps1_min = 195;
const uint16_t apps2_max = 775;
const uint16_t apps2_min = 440;
...
const double slip_ratio = 0.125;
double slip_ratio_current=0;
/* NDrive Parameters*/
...
//    Serial.print("enable_receive_right "); Serial.println(enable_receive_right);
    ...
    // // RPM
    //Serial.println( rpm_left_receive);
    //Serial.println(rpm_right_receive);
    // // Motor temperture
...
void set_avg_torque_tc(){
    // Map 1
    torque_avg = ecu_max_torque*apps_avg_frac*1.0;
    //torque_left = torque_avg;
    //torque_right = torque_avg;
    //torque_left_number = 100;
    rpm_rear=rpm_max*(min(rpm_left_receive,65767-rpm_left_receive))/32767;
    slip_ratio_current = (rpm_front-rpm_rear)/(rpm_front);
    if((slip_ratio_current<slip_ratio) && (torque_avg<torque_current)){
    ...
    else if((slip_ratio_current>slip_ratio) && (torque_avg<torque_current)){
      torque_left_number = (torque_avg/ndrive_max_torque)*fixed_rpm_max;
    //torque_right_number = 100;
      torque_right_number = (torque_avg/ndrive_max_torque)*fixed_rpm_max;
      torque_right_number=torque_left_number;
...
//          digitalWrite(AB,LOW);
          ...
          enable_sent_right = 1;
          enable_sent_left = 1;
          if ((rpm_front==0 && rpm_rear~=0) || (isnan(rpm_front)) || ((abs((rpm_front-rpm_rear)/rpm_rear))>0.2){    // Kept this here as then only it will follow the scrut point of disconnecting any one APPS and no torque should be commanded
        set_avg_torque();
    }
    else{
      set_avg_torque_tc();
...
//    digitalWrite(AB,HIGH);
  ...
  if((enable_receive_right == 1 || enable_receive_left == 1) && rtds){
    //Serial.print("\nSend torque \n");
    // optimisation(); //already optimised and normalised no need to do it again as there is a possibility it will interfeere with the HV ELEC scrut point of APPS 
    // normalisation();
    // if ((rpm_front==0 && rpm_rear~=0) || (isnan(rpm_front)) || ((abs((rpm_front-rpm_rear)/rpm_rear))>0.2){
    //     set_avg_torque();
    // }
    // else{
    //   set_avg_torque_tc();
