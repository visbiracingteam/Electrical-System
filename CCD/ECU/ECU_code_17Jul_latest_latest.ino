...
const int frg = 6; // Enable signal to be sent to MCs after completion of ignitiion sequence (rfe first) hold high until all good
const int rfe = 7;   // Enable signal to be sent to MCs after completion of ignitiion sequence (before frg) hold high until all good

/*Start-up variables*/
//uint16_t ignition;
bool rtds = false; //rtds status flag
// bool torque_cmd = false; // flag to comand torque only when apps is pressed, this will control rfe,frg and this will in turn control the enable signal

/*Sensor varialble*/
uint16_t apps1_in; // stores received apps1 value
uint16_t apps2_in; // stores received apps2 value

uint16_t apps1_in_opt = 110; // stores optimised apps1 value
uint16_t apps2_in_opt = 370; // stores optimised apps2 value
...
const uint16_t torque_threshold = 1; //When calculated torque is greater than this number then only rfe and frg will be set high
// float pressure_mapper(float pressure){ //takes input in form of bar and returns value in between 0 to 1024
//   float pressure_normalized = ((pressure*4/138)+0.5);
//   return(pressure_normalized *1024/5);
// }
...
/*Range of sensors*/
const uint16_t apps1_gmax = 950; //Value of apps1 greater than gmax then apps_range_error will trigger, this means the value of apps is pulled up
const uint16_t apps1_gmin = 0; //Value of apps1 less than gmin then apps_range_error will trigger, this means the value of apps is pulled down or shorted to ground
const uint16_t apps2_gmax = 1010; //Value of apps2 greater than gmax then apps_range_error will trigger, this means the value of apps is pulled up
const uint16_t apps2_gmin = 0; //Value of apps2 less than gmin then apps_range_error will trigger, this means the value of apps is pulled down or shorted to ground

const uint16_t bps1_gmax = 950; //Value of bps1 greater than gmax then bps_range_error will trigger, this means the value of bps is pulled up
const uint16_t bps1_gmin = 10; //Value of bps1 less than gmin then bps_range_error will trigger, this means the value of bps is pulled down or shorted to ground
const uint16_t bps2_gmax = 1000; //Value of bps2 greater than gmax then bps_range_error will trigger, this means the value of bps is pulled up
const uint16_t bps2_gmin = 10; //Value of bps2 less than gmin then bps_range_error will trigger, this means the value of bps is pulled down or shorted to ground

/*Working range of sensors (calibratiion)*/
const uint16_t apps1_max = 750; //max value used for calibration
const uint16_t apps1_min = 110; //min value used for calibration
const uint16_t apps2_max = 980; //max value used for calibration
const uint16_t apps2_min = 365; //min value used for calibration
...
bool apps_range_error(){
  if((apps1_in < apps1_gmin) || (apps1_in > apps1_gmax) || (apps2_in < apps2_gmin) || (apps2_in > apps2_gmax) ){
    Serial.println("APPS RANGE ERROR");
    Serial.println("+");
    Serial.println("+");
    Serial.println("+");
    Serial.println("+");
...
bool bps_range_error(){
  if((bps1_in < bps1_gmin) || (bps1_in > bps1_gmax) || (bps2_in > bps2_gmax) || (bps2_in < bps2_gmin)){
    Serial.println("BPS RANGE ERROR");
    Serial.println("-");
    Serial.println("-");
    Serial.println("-");
    Serial.println("-");
...
void optimisation(){
  ...
  apps1_in_opt = apps1_max;
  if(apps1_in < apps1_min)
  apps1_in_opt = apps1_min;
  if(apps2_in > apps2_max)
  apps2_in_opt = apps2_max;
  if(apps2_in < apps2_min)
  apps2_in_opt = apps2_min;

  if(bps1_in < bps1_min)
  bps1_in = bps1_min;
  //Serial.print("bps_optimize");
  if(bps2_in < bps2_min)
  bps2_in = bps2_min;
//  if(bps1_in > bps1_max)
//  bps1_in = bps1_max;
...
void normalisation(){
  apps1_normalised = 0.95*(apps1_in_opt - apps1_min)/apps1_range;
 // Serial.println(apps1_normalised);
  apps2_normalised = 0.95*(apps2_in_opt - apps2_min)/apps2_range;
...
const double ecu_max_torque = 55;  // Nm
...
void set_avg_torque(){
    ...
    //torque_left_number = 100;
      torque_left_number = (torque_avg/ndrive_max_torque)*32767;
    //torque_right_number = 100;
      torque_right_number = (torque_avg/ndrive_max_torque)*32767;
      torque_right_number=torque_left_number;
...
//bool brake_pressed(){
 ...
 //   return true;
  ...
  //}
  //else if(bps2_in > bps_threshold)
  //{
    //Serial.println("Bps2 pressed");
   // return false;
  //}
  //else
  //{ 
...
//  ignition = digitalRead(ignition_sig);
  if(!rtds && (digitalRead(ignition_sig) == LOW) && brake_pressed() && !(apps_range_error()) && (!(bps_range_error()))){ //Logic Reversed
...
void on(){
  if(torque_avg > torque_threshold){
...
//  pinMode(AE, OUTPUT);
//  pinMode(AB, OUTPUT);
  attachInterrupt(digitalPinToInterrupt(c_plus), precharge, CHANGE); //whenever there is a change in the value of c_plus, precharge function is called
...
//  attachInterrupt(digitalPinToInterrupt(dr_select), change_max_torque, CHANGE);
  ...
  /*Can setup*/
  //Control CAN
  c_can.begin();//C_CAN bus initializtion
...
void loop(){
  ...
  // }
  Serial.print("bps_thr:");
  Serial.println(bps_thr);
  //bps_threshold = pressure_mapper(bps_thr);
  Serial.print("bps_threshold:");
  Serial.println(bps_threshold);
  ...
  delay(10);
  //apps2_in -= apps2_sub;
  apps1_in_opt = apps1_in;
  apps2_in_opt = apps2_in;
  ...
  if(!(apps_range_error()) && (!(bps_range_error()))){
    ...
    if(precharge()){
      ...
      //   normalisation();
        ...
        Serial.print("apps_10percent:");
        Serial.println();
        set_avg_torque();
//         if(hb_flag){
//           Serial.println("Release the brakes and accelerator");
//           torque_left_number = 0;f
//           torque_right_number = 0;
// //          digitalWrite(AD,LOW);
// //          digitalWrite(AE,LOW);
...
//    digitalWrite(AB,HIGH);
  ...
  //   send_right();
  // }
  if(rtds && !(apps_range_error()) && !(bps_range_error())  ){
    //Serial.print("\nSend torque \n");
    Serial.println("RTDS present | no apps and bps range error");
    send_left();
    send_right();
  }
  else{
...
//Serial.println(bps2_in);
  ...
  Serial.println();
  Serial.print("apps1 optimised:");
  Serial.println(apps1_in_opt);
  Serial.println();
  Serial.print("apps1_normalised - apps2_normalised:");
  ...
  Serial.println();
  Serial.print("apps2 optimised:");
  Serial.println(apps2_in_opt);
  Serial.println();
  Serial.print("bps1:");
  Serial.println(bps1_in);
  Serial.println();
  ...
  Serial.println(bps2_in);
  Serial.println();
  int rfe_read = digitalRead(rfe);
  int frg_read = digitalRead(frg);
  int cplus_read = digitalRead(c_plus);
  int rtds_read = digitalRead(rtds_sig);
  Serial.print("Rfe:");
  Serial.println(rfe_read);
  Serial.print("frg:");
  Serial.println(frg_read);
  Serial.print("enable_receive_right:");
  Serial.println(enable_receive_right);
  Serial.print("enable_receive_left:");
  Serial.println(enable_receive_left);
  Serial.print( "motor_temp_left_receive:");
  Serial.println( motor_temp_left_receive);
//  Serial.println(rtds);
