...
inline bool err_detected(byte error) {
  return error > 0x3;
}
...
void loop() {
  ...
  if (loopcount) {
    if (slowChannelReady) {
      str += String((unsigned int)((LTC2949_GetLastTBxInt() - deltaT) * (1.0e3 * LTC2949_LSB_TB1)));
    }  //Serial.print((unsigned int)((LTC2949_GetLastTBxInt() - deltaT) * (1.0e3 * LTC2949_LSB_TB1)));
    else {}
  }
#endif
...
void triggerchargerError(void) {
  ...
  if (output_voltage > 250 && chargingStarted_) {
    ...
    if (can3.read(receive_msg)) {
      ...
      Serial.println(receive_msg.id, HEX);
      Serial.print("LEN: ");
      Serial.println(receive_msg.len);
      output_voltage = (receive_msg.buf[0] * 256 + receive_msg.buf[1] + 1.5) / 10;
      output_current = (receive_msg.buf[2] * 256 + receive_msg.buf[3]) / 10;
      Serial.println("Attempting to start charging");
      Serial.print("Output Voltage [CHARGER] : ");
      Serial.println(output_voltage);
      Serial.print("Output Current [CHARGER] : ");
