...
uint16_t bps threshold = 150; //Only defining variable here, will be updated using bps_thr abd pressure_mapper function
...
const uint16_t bps1_gmin = 101; //Value of bps1 less than gmin then bps_range_error will trigger, this means the value of bps is pulled down or shorted to ground
...
const uint16_t bps2_gmin = 100; //Value of bps2 less than gmin then bps_range_error will trigger, this means the value of bps is pulled down or shorted to ground

/*Working range of sensors (calibratiion)*/
const uint16_t apps1_max = 690; //max value used for calibration
const uint16_t apps1_min = 50; //min value used for calibration
const uint16_t apps2_max = 880; //max value used for calibration
const uint16_t apps2_min = 200; //min value used for calibration
...
void set_avg_torque(){
    ...
    torque_left_number = (torque_avg/ndrive_max_torque)*fixed_rpm_max;
    //torque_right_number = 100;
    torque_right_number = (torque_avg/ndrive_max_torque)*fixed_rpm_max;
...
bool brake_pressed(){
  ...
  elif((bps2_in > bps_threshold) || (bps_range_error()))
  {
...
//Serial.println(bps2_in);
  ...
  Serial.print("bps1_gmin:");
  Serial.println(bps1_gmin);
  Serial.print("bps2_gmin:");
  Serial.println(bps2_gmin);
  Serial.print("apps1_gmin:");
  Serial.println(apps1_gmin);
  Serial.print("apps2_gmin:");
  Serial.println(apps2_gmin);
